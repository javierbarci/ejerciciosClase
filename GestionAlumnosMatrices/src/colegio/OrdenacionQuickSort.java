...
public class OrdenacionQuickSort {
    ...
    private static void quickSort(int menor, int mayor) {
        ...
        do {
            for(; i<pivote && 
                alumnos[i].getNombre().compareToIgnoreCase(
                    alumnos[pivote].getNombre())<0; i++);
            for(; j>i && 
                alumnos[j].getNombre().compareToIgnoreCase(
                    alumnos[pivote].getNombre())>0; j--);
            if (i<j){
